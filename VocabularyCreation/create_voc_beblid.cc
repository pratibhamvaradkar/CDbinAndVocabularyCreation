...
int main()
{
    ...
    for (size_t i=0; i<count; i++) //Looping through images
    {
        ...
        if (index>0)
        {
            ...
            cv::Mat descriptors;
            auto scaleFactor = 1.00; ///for ORB
            int n_bits = cv::xfeatures2d::BEBLID::SIZE_256_BITS;
            auto desc = cv::xfeatures2d::BEBLID::create(scaleFactor, n_bits);
            desc->compute(image_now, kypts, descriptors);
