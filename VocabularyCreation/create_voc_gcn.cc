...
int main()
{
    ...
    torch::jit::script::Module module = torch::jit::load("../gcn2/gcn2.pt");
    // std::shared_ptr<torch::jit::script::Module> module = torch::jit::load("../gcn2.pt");
    ...
    for (size_t i=0; i<count; i++)
    {
        ...
        if (index>0)
        {
            ...
            auto img1_tensor = torch::from_blob(img1.data, {1,img_height, img_width, 1}).to(torch::kCPU);
